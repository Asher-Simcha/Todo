...
showweathernow(){
    ...
    echo '${color4}${voffset -5}${font Infini:pixelsize=20}Fortune'
    echo '${color1}${voffset -10}${font Infini:pixelsize=13}${exec cat '$FILENAMEFORTUNE' | fold -w 40 -s}'
    echo '${color}'
}
showweathernow(){
    SHOWENDING=1
    local THISWEATHER=$($WEATHER)
    echo '${color4}${font Infini:pixelsize=20}Weather${font Infini:pixelsize=20}${color2}CPU ${voffset 12}${font Infini:pixelsize=19}'$THISWEATHER
}
events(){
    if [ -e $EVENT1  ]; then
        SHOWENDING=1
        echo '${color4}${voffset -5}${font Infini:pixelsize=20}Events'
        echo '${color1}${voffset -15}${font Infini:pixelsize=13}${exec  cat '$EVENT1' | fold -w 40 -s}'
        if [ -e $EVENT1 ]; then
#             echo '${color4}${voffset 5}${font Infini:pixelsize=20}Event 2'
            echo '${color1}${voffset -5}${font Infini:pixelsize=13}${exec cat '$EVENT2' | fold -w 40 -s}'
        fi
        echo '${color}'
    fi
}
notes(){
    if [ -e $NOTE1  ]; then
        SHOWENDING=1
        echo '${color5}${voffset -5}${voffset -5}${font Infini:pixelsize=20}Notes'
        echo '${color1}${voffset -15}${font Infini:pixelsize=13}${exec cat '$NOTE1' | fold -w 40 -s}'
        if [ -e $NOTE2 ]; then
#             echo '${color5}${voffset 5}${font Infini:pixelsize=20}Note 2'
            echo '${color1}${voffset -5}${font Infini:pixelsize=13}${exec cat '$NOTE2' | fold -w 40 -s}'
        fi
        echo '${color}'
    fi
}
tasks(){
    if [ -e $TASK1  ]; then
        SHOWENDING=1
        echo '${color6}${voffset -5}${voffset -5}${font Infini:pixelsize=20}Tasks'
        echo '${color1}${voffset -15}${font Infini:pixelsize=13}${exec cat '$TASK1' | fold -w 40 -s}'
        if [ -e $TASK2 ]; then
            echo '${color1}${voffset -5}${font Infini:pixelsize=13}${exec cat '$TASK2' | fold -w 40 -s}'
        fi
        if [ -e $TASK3 ]; then
            echo '${color1}${voffset -5}${font Infini:pixelsize=13}${exec cat '$TASK3' | fold -w 40 -s}'
        fi
        if [ -e $TASK4 ]; then
            echo '${color1}${voffset -5}${font Infini:pixelsize=13}${exec cat '$TASK4' | fold -w 40 -s}'
        fi
        if [ -e $TASK5 ]; then
            echo '${color1}${voffset -5}${font Infini:pixelsize=13}${exec cat '$TASK5' | fold -w 40 -s}'
        fi
    fi
}
displayheader(){
    echo '${color3}${font Infini:pixelsize=20}${hr 1}';
}
header()
{
    HEADERSHOW=0;
