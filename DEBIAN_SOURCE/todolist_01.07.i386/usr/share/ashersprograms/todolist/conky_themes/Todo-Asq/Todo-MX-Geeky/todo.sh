...
showfortuneNow(){
    SHOWENDING=1
    echo '${color4}${voffset 3}${font Droid Sans:size=7}${exec cat '$FILENAMEFORTUNE' | fold -w 40 -s}'
}
showweathernow(){
    local THISWEATHER;
    SHOWENDING=1
    ...
    echo '${color6}${font Droid Sans:size=7}Weather ${color4}${alignr}${font Droid Sans:size=7}'$THISWEATHER
}
events(){
    if [  -e $EVENT1 ]; then
        SHOWENDING=1
        echo '${color6}${voffset 5}${font Droid Sans:size=7}Events'
        echo '${color4}${voffset 3}${font Droid Sans:size=7}${exec  cat '$EVENT1' | fold -w 40 -s}'
        if [ -e $EVENT2 ]; then
            echo '${color4}${voffset 3}${font Droid Sans:size=7}${exec cat '$EVENT2' | fold -w 40 -s}'
        fi
    fi
}
notes(){
    if [  -e $NOTE1 ]; then
        SHOWENDING=1
        echo '${color7}${voffset 5}${font Droid Sans:size=7}Notes'
        echo '${color4}${voffset 3}${font Droid Sans:size=7}${exec cat '$NOTE1' | fold -w 40 -s}'
        if [ -e $NOTE2 ]; then
            echo '${color4}${voffset 3}${font Droid Sans:size=7}${exec cat '$NOTE1' | fold -w 40 -s}'
        fi
    fi
...
tasks(){
    if [  -e $TASK1 ]; then
        SHOWENDING=1
        echo '${color8}${voffset 5}${font Droid Sans:size=7}Tasks'
        echo '${color4}${voffset 3}${font Droid Sans:size=7}${exec cat '$TASK1' | fold -w 40 -s}'
        if [ -e $TASK2 ]; then
            echo '${color4}${voffset 3}${font Droid Sans:size=7}${exec cat '$TASK2' | fold -w 40 -s}'
        fi
        if [ -e $TASK3 ]; then
            echo '${color4}${voffset 3}${font Droid Sans:size=7}${exec cat '$TASK3' | fold -w 40 -s}'
        fi
        if [ -e $TASK4 ]; then
            echo '${color4}${voffset 3}${font Droid Sans:size=7}${exec cat '$TASK4' | fold -w 40 -s}'
        fi
        if [ -e $TASK5 ]; then
            echo '${color4}${voffset 3}${font Droid Sans:size=7}${exec cat '$TASK5' | fold -w 40 -s}'
        fi
    fi
}
displayheader(){
    echo '${color2}Todo List ${hr 2}${color}';
}
header()
{
    HEADERSHOW=0;
