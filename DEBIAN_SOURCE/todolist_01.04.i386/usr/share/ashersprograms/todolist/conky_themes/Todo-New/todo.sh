...
# Source: 
# Additional_Sources: 
# License: copyright ashersprograms.com
# Additional_Licenses: 
# Credits: 
...
CACHE_LOCATION="$HOME/.cache/todo_list/"
...
#FILENAMEWEATHER=$CACHE_LOCATION"Weather.txt"
...
TODO_CHILD_CFG_FILE="$CFG_LOCATION/todosh.ini"
...
checkfilesystem(){
    if [ -e $TODO_MAIN_CFG_FILE ]; then
        ...
        if [ ! -e $TODO_CHILD_CFG_FILE ]; then
            $($PROGRAM_INI $PROGRAM_INI $f $TODO_CHILD_CFG_FILE $w $cfg $v $one)
            getweather
        fi
    else
        exit 1;
...
getweather(){
    ...
    $($PROGRAM_INI -f $TODO_CHILD_CFG_FILE -w $tw -v "${THISWEATHER}")
}
createfortune(){
...
showfortuneNow(){
    echo '${goto $alignc}${color3}${font ubuntu:pixelsize=20}FORTUNE'
    echo '${goto $alignc}${voffset -5}${color3}${hr 2}'
    echo '${color5}${voffset -10}${font ubuntu:pixelsize=13}${exec cat '$FILENAMEFORTUNE' | fold -w 40 -s}'
    echo '${color}'
    echo ""
}
showweathernow(){
    local THISWEATHER=$($PROGRAM_INI $f $TODO_CHILD_CFG_FILE $r $tw)
    echo '${goto $alignc}${color3}${font ubuntu:pixelsize=20}WEATHER'
    echo '${goto $alignc}${voffset -5}${color3}${hr 2}'
    echo '${color5}${voffset -3}${font ubuntu:pixelsize=13}'$THISWEATHER
    echo '${color}'
}
events(){
if [  -e $EVENT1 ]; then
    echo '${goto $alignc}${color3}${font ubuntu:pixelsize=20}EVENTS'
    echo '${goto $alignc}${voffset -5}${color3}${hr 2}'
    echo '${color4}${voffset 5}${font ubuntu:pixelsize=15}Event 1'
    echo '${color5}${voffset -3}${font ubuntu:pixelsize=13}${exec  cat '$EVENT1' | fold -w 40 -s}'
    if [ -e $EVENT2 ]; then
        echo '${color4}${voffset 5}${font ubuntu:pixelsize=15}Event 2'
        echo '${color5}${voffset -3}${font ubuntu:pixelsize=13}${exec cat '$EVENT2' | fold -w 40 -s}'
    fi
    echo '${color}'
fi
}
notes(){
if [  -e $NOTE1 ]; then
    echo '${goto $alignc}${color3}${font ubuntu:pixelsize=20}NOTES'
    echo '${goto $alignc}${voffset -5}${color3}${hr 2}'
    echo '${color4}${voffset 5}${font ubuntu:pixelsize=15}Note 1'
    echo '${color5}${voffset -3}${font ubuntu:pixelsize=13}${exec cat '$NOTE1' | fold -w 40 -s}'
    if [ -e $NOTE2 ]; then
        echo '${color4}${voffset 5}${font ubuntu:pixelsize=15}Note 2'
        echo '${color5}${voffset -3}${font ubuntu:pixelsize=13}${exec cat '$NOTE2' | fold -w 40 -s}'
    fi
    echo '${color}'
fi
}
tasks(){
if [  -e  $TASK1 ]; then
    echo '${goto $alignc}${color3}${font ubuntu:pixelsize=20}TASKS'
    echo '${goto $alignc}${voffset -5}${color3}${hr 2}'
    echo '${color4}${voffset 5}${font ubuntu:pixelsize=15}Task 1'
    echo '${color5}${voffset -3}${font ubuntu:pixelsize=13}${exec cat '$TASK1' | fold -w 40 -s}'
    if [ -e $TASK2 ]; then
        echo '${color4}${voffset 5}${font ubuntu:pixelsize=15}Task 2'
        echo '${color5}${voffset -3}${font ubuntu:pixelsize=13}${exec cat '$TASK2' | fold -w 40 -s}'
    fi
    if [ -e $TASK3 ]; then
        echo '${color4}${voffset 5}${font ubuntu:pixelsize=15}Task 3'
        echo '${color5}${voffset -3}${font ubuntu:pixelsize=13}${exec cat '$TASK3' | fold -w 40 -s}'
    fi
    if [ -e $TASK4 ]; then
        echo '${color4}${voffset 5}${font ubuntu:pixelsize=15}Task 4'
        echo '${color5}${voffset -3}${font ubuntu:pixelsize=13}${exec cat '$TASK4' | fold -w 40 -s}'
    fi
     if [ -e $TASK5 ]; then
        echo '${color4}${voffset 5}${font ubuntu:pixelsize=15}Task 5'
        echo '${color5}${voffset -3}${font ubuntu:pixelsize=13}${exec cat '$TASK5' | fold -w 40 -s}'
    fi
    echo '${color}'
fi
}
